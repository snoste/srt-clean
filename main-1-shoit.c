...
void *trace_thread(void *f){
	struct passToThread_struct *thread_f = f;
  trace( &thread_f->sceneT, thread_f->sample_colorT, &thread_f->pixel_rayT, 0);
	pthread_exit(0);
}
...
{
	pthread_t trace_thread_variable;
			...
			for( int xs=-halfSamples; xs<=halfSamples; ++xs )
			{
				for( int ys=-halfSamples; ys<=halfSamples; ++ys )
				{
					 ...
					 */
					 ...
					 struct passToThread_struct f;
					 f.sceneT = scene;
					 copy(f.sample_colorT, sample_color);
					 f.pixel_rayT = pixel_ray;
					 /* create a thread which executes jpeg_finish_compress(&info) */
				   if(pthread_create(&trace_thread_variable, NULL, trace_thread, (void *)&f)) {
				     fprintf(stderr, "Error creating thread\n");
				     return 1;
				   }
					 /* Original function call
					 trace( &scene, sample_color, &pixel_ray, 0 );*/
					 /* wait for the compress thread to finish */
				   if(pthread_join(trace_thread_variable, NULL)) {
